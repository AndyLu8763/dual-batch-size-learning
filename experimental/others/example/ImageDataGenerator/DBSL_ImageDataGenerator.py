...
# clear; python experimental/DBSL_development.py -a='140.109.23.144' -r= &
# server: gpu01
# worker: gpu02, gpu03, gpu04, gpu05
...
from torch.distributed.rpc import rpc
...
print(f'Device: {physical_devices[device_index]}')
...
class ParameterServer(object):
    ...
    def get_mission_model_bs_lr(ps_rref, worker_rank):
        ...
        self.count_epoch()
        return self.mission_complete, self.model.get_weights(), self.batch_size[worker_rank], self.learning_rate[worker_rank]
    ...
    def push_and_pull_model(ps_rref, worker_weights, worker_batch_size, worker_rank, epoch):
        ...
        with self.model_lock:
            ...
            for i in range(len(server_weights)):
                server_weights[i] = ((2 - update_factor) * server_weights[i] + update_factor * worker_weights[i]) / 2
            self.model.set_weights(server_weights)
            return self.model.get_weights()

    def record_loss_acc(ps_rref, worker_rank, epoch, train_loss, train_acc, test_loss, test_acc):
        self = ps_rref.local_value()
    ...
    def save_history(self):
        ...
        fname = f'{num_GPU}GPU_{extra_time_ratio}extra_{num_small}small_{small_BS}batchSize'
...
class Worker(object):
    ...
    def print_loss_acc(
        self, epoch, batch_size, lr, train_loss, train_acc, test_loss, test_acc
    ):
        print('--------')
        ...
        for epoch in range(record):
            ...
            self.model.compile(
                optimizer=keras.optimizers.SGD(
                    learning_rate=learning_rate, momentum=momentum, decay=decay
                ),
            ...
            weights = rpc.rpc_sync(
                ...
                args=(
                    self.ps_rref, self.model.get_weights(), batch_size, self.rank, epoch
                )
            )
            self.model.set_weights(weights)
            # eval
            ...
            rpc.rpc_sync(
                ...
                args=(
                    self.ps_rref, self.rank, epoch, train_loss, train_acc, test_loss, test_acc
                )
            )
            self.print_loss_acc(
                epoch, batch_size, learning_rate, train_loss, train_acc, test_loss, test_acc
            )
...
def run_program(rank, world_size, addr, port):
    ...
    backend_options = rpc.TensorPipeRpcBackendOptions(
        init_method=f'tcp://{addr}:{port}', rpc_timeout=0
    )
    if rank:           # worker
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Efficient Dual Batch Size Deep Learning for Distributed Parameter Server Systems'
    )
    parser.add_argument(
        '-r', '--rank',
        type=int,
        default=None,
        help='Global rank of the process, pass in 0 for the master and others for workers.'
    )
    parser.add_argument(
        '-w', '--world_size',
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default='8763',
        help='Port that the master is listening on, default "8763".'
    )
