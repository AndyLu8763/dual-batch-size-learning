...
# clear; python experimental/1080/DBSL_1080.py -a='140.109.23.144' -s='' -r= &
# server: gpu01
...
class ParameterServer(object):
    def __init__(self, world_size, sync_scheme):
        # others
    ...
    def save_history(self):
        ...
        fname = (
            f'SSP{SSP_threshold}_{num_GPU}GPU_{extra_time_ratio}extra_{num_small}small_{small_BS}batchSize'
            if self.sync_scheme == 'SSP'
            else f'{self.sync_scheme}_{num_GPU}GPU_{extra_time_ratio}extra_{num_small}small_{small_BS}batchSize'
        )
        # load: npy = np.load('filename.npy', allow_pickle=True)
        # read: npy.item()['xxx']
        np.save(f'results/npy/{fname}.npy', content)
        # load: model = keras.models.load_model('directory_path')
        # The warning causes since there is no {model.compile()} in the server code.
        self.model.save(f'results/model/{fname}')
...
################################################################
def run_parameter_server(world_size, sync_scheme):
    ps_rref = rpc.RRef(ParameterServer(world_size, sync_scheme))
    future_list = []
...
################################################################
def run_program(rank, world_size, addr, port, sync_scheme):
    ...
    if rank:           # worker
        ...
        if sync_scheme == 'A':
            ...
            sync_scheme = 'SSP'
        print(f'Server {rank} initializing RPC, the synchronization scheme is "{sync_scheme}"')
        rpc.init_rpc(
            name=f'server_{rank}',
            rank=rank,
            world_size=world_size,
            rpc_backend_options=backend_options)
        run_parameter_server(world_size, sync_scheme)
    rpc.shutdown()
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-s', '--scheme',
        type=str,
        help='The synchronization scheme, should be "ASP", "BSP", "SSP" or "A", "B", "S".'
    )
    
    args = parser.parse_args()
    schemes = ['A', 'B', 'S', 'ASP', 'BSP', 'SSP']
    ...
    assert args.scheme in schemes, 'The scheme must be "ASP", "BSP", "SSP" or "A", "B", "S".'

    run_program(args.rank, args.world_size, args.master_addr, args.master_port, args.scheme)
