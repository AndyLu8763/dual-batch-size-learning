...
class Server(object):
    def __init__(self, args):
        ...
        #self.epochs = 100 if 'cifar' in args.dataset else 70   # testing, 140 -> 100, 105 -> 70
        ...
        if args.dataset == 'cifar10' or args.dataset == 'cifar100':
            ...
            if args.amp:
                self.intercept_ls = (
                    [0.010680175780982126] if args.xla
                    else [0.016659967832795497]
                )
                self.coef_ls = (
                    [0.0007576280737663753] if args.xla
                    else [0.0009387165655225843]
                )
                self.large_batch_size_ls = [900] if args.xla else [740]
            else:
                raise ValueError('The ImageNet training process only supports "--amp"')
            self.resolution_ls = [288]
            self.dropout_rate_ls = [0.3]
        else:
