...
class Server(object):
    def __init__(self, args):
        ...
        if args.dataset == 'cifar10' or args.dataset == 'cifar100':
            ...
            if args.amp:
                self.intercept_ls = (
                    [5.683134939754586e-05, 0.01073572758256125] if args.xla
                    else [0.012036769987567586, 0.01705088976279029]
                )
                self.coef_ls = (
                    [0.00025895413494090355, 0.00045862692849402506] if args.xla
                    else [0.0003013537830804257, 0.0005683767808602767]
                )
                self.large_batch_size_ls = [2800, 1400] if args.xla else [2330, 1110]
            else:
                raise ValueError('The ImageNet training process only supports "--amp"')
            self.resolution_ls = [160, 224]
            self.dropout_rate_ls = [0.1, 0.2]
        else:
        ...
        self.outfile = (
            f'stage_{args.dataset}_resnet{args.depth}_e{self.epochs}'
